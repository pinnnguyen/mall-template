<template>
  <view class="wrap">
    <swiper class="swiper" :style="{ height: swiperHeight + 'px' }" :interval="interval" :duration="duration"
      ...
      <swiper-item v-for="(item, index) in list" :key="index" @click="$emit('clickItem', item)">
        <view v-if="list && list.length > 0" class="item"
          ...
          <div :id="'swiper-item' + index">
            <image v-if="!slots" ref="swiper-item" class="item-img" :class="[imgShadow ? 'imgShadow' : '']"
              :src="item[imgKey]" :style="{ borderRadius: imgRadius + 'px', width: imgWidth + '%' }" mode="widthFix"
              @load="load(index)"></image>
...
export default {
  props: {
    ...
    // Khoảng cách dưới cùng của điểm phát sóng bánh xe
    bottom: {
      type: Number,
      default: 10,
    },
    // Hiệu ứng đặc biệt thẻ
    crown: {
      type: Boolean,
      default: false,
    },
